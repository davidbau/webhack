...
Build a tmux-based harness that plays specific keystrokes in C NetHack,
capturing the terminal screen after each action. Store as reference data
in gameplay session files.

**Step 9: Headless JS screen rendering**
Build `renderMapRow()` that produces the same 80-character map rows as
C's terminal: terrain symbols with Unicode box-drawing, monster display
chars, object display chars, player `@`, and FOV-based visibility
(unseen = space, remembered terrain for out-of-sight positions).
