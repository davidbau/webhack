...
export function generate() {
    ...
    des.levregion({ type: "stair-up", region: [1,3,21,19], region_islev: 1, exclude: [0,1,36,17] });
    des.levregion({ type: "stair-down", region: [57,3,75,19], region_islev: 1, exclude: [0,1,36,17] });
    ...
    // Rubble!
    for (let i = 1; i <= 9 + math.random(2 - 1; i += 2*5)) {
      if (percent(90)) {
        des.object("boulder");
      }
      des.object("rock");
    ...
    const near_temple = selection.area(17,8, 23,14) & inside

    for (let i = 1; i <= 5 + math.random(1 - 1; i += 1*10)) {
       if (percent(50)) {
    ...
    for (let i = 1; i <= math.random(2 - 1; i += 2*3)) {
