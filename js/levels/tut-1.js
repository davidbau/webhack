/**
 * tut-1 - NetHack special level
 * Converted from: tut-1.lua
 */
...
export function generate() {
    ...
    const tut_ctrl_key = null;
    const tut_alt_key = null;

    function tut_key(command) {
       const s = nh.eckey(command);
       const m = s.match("^^([A-Z])$"); -- ^X is Ctrl-X
       if ((m !== null)) {
          tut_ctrl_key = m
          return "Ctrl-" + m;;
       }
    ...
    const armor = (u.role === "Monk") && "leather gloves" || "leather armor";
    ...
    des.object({ coord: [41,6], id: "large box", broken: true, trapped: false, contents: function(obj) { des.object({ id: "secret door detection", class: "/", spe: 30 }); } });
