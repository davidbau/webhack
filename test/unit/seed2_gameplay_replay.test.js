...
const sessionPath = join(__dirname, '../comparison/sessions/seed2_wizard_fountains_gameplay.session.json');
const session = existsSync(sessionPath)
    ? JSON.parse(readFileSync(sessionPath, 'utf-8'))
    : null;

describe('C gameplay replay: seed2 wizard fountains', { skip: !session }, () => {
    if (!session) return;
